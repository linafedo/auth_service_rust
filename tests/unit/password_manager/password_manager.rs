...
mod password_manager_tests {
    ...
    fn check_password_success() {
        ...
        let password_data = password_data::generate(password.expose_secret()).unwrap();

        let result = password_data::check_password(
            &password_str,
    ...
    fn generate_password_data () {
        let password_str = "123456";
        let password = Password::parse(password_str.to_string()).unwrap();
        let password_data = password_data::generate(password.expose_secret());
